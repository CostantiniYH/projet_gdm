    ...
    public function edit() {
        try {
            if (!empty($_POST['matiere']) && empty($_POST['theme'])) {
                $nom = htmlspecialchars($_POST['matiere']) ?? null;
                $this->modelMatiere->insertMatiere($nom);
                header("Location: /forms?success=1");
                exit;
            }
             if (!empty($_POST['theme']) && !empty($_POST['matiere'])) {
                $nom = htmlspecialchars($_POST['theme'] ?? null);
                $matiere_id = htmlspecialchars($_POST['matiere'] ?? null);
